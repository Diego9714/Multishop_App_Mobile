...
const formatNumber = (number, currency) => {
  ...
  let formattedNumber = numberFormat.toLocaleString('es-ES', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
                ...
                applyPagination().map((payment) => (
                  <View key={payment.id_pass} style={styles.productItem2}>
                    ...
                    <View style={styles.priceContainer}>
                      <Text>{formatDate(new Date(payment.fecha))}</Text>
                      <Text>{formatNumber(payment.amount, payment.currency_type)}</Text>
                    </View>
                  </View>
                ))
              ) : (
                <Text style={styles.noOrdersText}>No se encontraron abonos</Text>
              )}
            </ScrollView>
          </View>
